...
def process_data(file):
    try:
        # 1. 일단 데이터의 앞부분만 읽어서 헤더(제목 줄)가 어디 있는지 찾습니다.
        # '번 호' 또는 '성 명'이라는 단어가 있는 행을 헤더로 인식합니다.
        temp_df = pd.read_excel(file, header=None, nrows=10)
        header_row_index = -1
        
        for i, row in temp_df.iterrows():
            row_values = row.astype(str).values
            if any("번 호" in s or "성 명" in s for s in row_values):
                header_row_index = i
                break
        
        if header_row_index == -1:
            return None, "표의 헤더('번 호', '성 명')를 찾을 수 없습니다."

        # 2. 찾은 위치를 기준으로 다시 파일을 읽습니다.
        file.seek(0) # 파일 커서를 처음으로 돌림
        df = pd.read_excel(file, header=header_row_index)

        # 3. 데이터 클렌징
        # 번호와 성명의 빈칸을 채웁니다 (위의 값으로 채우기)
        # 컬럼명에 공백이 있을 수 있으므로 공백을 제거하고 비교합니다.
        df.columns = [c.replace(" ", "") if isinstance(c, str) else c for c in df.columns]
        
        # 필수 컬럼 확인
        if '번호' not in df.columns or '성명' not in df.columns:
             return None, "'번호' 또는 '성명' 컬럼을 찾을 수 없습니다."

        df['번호'] = df['번호'].fillna(method='ffill')
        df['성명'] = df['성명'].fillna(method='ffill')

        # 4. 내용 합치기
        # 번호와 성명을 제외한 나머지 텍스트 컬럼들을 합칩니다.
        # 예: 과목명이 있으면 [국어] 내용... 형태로 만들어주면 더 좋습니다.
        
        # 합칠 대상 컬럼 찾기 (번호, 성명, 학년, 반, 학기 등을 제외한 긴 텍스트 컬럼)
        exclude_cols = ['번호', '성명', '학년', '반', '학기', '이수단위', '원점수', '과목', '성취도/석차등급']
        ...
        # 그룹화하여 병합
        # '과목' 컬럼이 있다면 과목도 같이 보여주는 것이 좋으므로 로직 추가
        if '과목' in df.columns:
             df['내용병합'] = "[" + df['과목'].fillna("") + "] " + df[target_cols[0]].astype(str)
        ...
        result_df = df.groupby(['번호', '성명'])[target_col_name].apply(merge_text).reset_index()
...
if uploaded_file:
    ...
    if error_msg:
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            ...
            worksheet.set_column('C:C', 50, format_text) # 내용 컬럼 너비 조절
